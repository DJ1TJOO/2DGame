    ...
    constructor(gl, vs, fs) {
        ...
        if (vsShader && fsShader) {
            ...
            if (!gl.getProgramParameter(this.program, gl.LINK_STATUS)) {
                console.error("Cannot leader shader \n" + gl.getProgramInfoLog(this.program));
                return null;
            }

            this.getherParameters();
...
class Sprite {
    ...
    static createRenderRectArray(x = 0, y = 0, w = 1, h = 1) {
        return new Float32Array([
            x, y,
            x + w, y,
            x, y + h,
    ...
    render(position, frames, worldSpaceMatrix, options = {}) {
        if (this.isLoaded) {
            let gl = this.gl;
...
class BackBuffer {
    constructor(gl, options) {
        this.gl = gl;
        this.material = new Material(gl, vs_backbuffer, fs_backbuffer);
